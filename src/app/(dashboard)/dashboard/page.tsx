  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-fade-up">
      ...
      <div className="grid gap-4 sm:gap-6 lg:grid-cols-2">
          ...
          <CardContent className="space-y-1">
            {recentTenders && recentTenders.length > 0 ? (
              <>
                ...
                <Button variant="ghost" size="sm" className="w-full mt-3 sm:hidden" asChild>
                  <Link href="/tenders">
        ...
        <Card className="lg:col-span-2 card-hover border-orange-500/20">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-lg flex items-center gap-2">
          ...
          <CardContent>
            ...
            <Button variant="ghost" size="sm" className="w-full mt-4 sm:hidden" asChild>
              <Link href="/tenders?closing_soon=true">
...
function StatCard({ 
  ...
  return (
    <Card className={`card-hover ${urgent ? 'border-orange-500/50 bg-orange-500/5' : ''}`}>
      <CardContent className="p-4 sm:p-6">
        <div className="flex items-start justify-between">
          <div className="space-y-1">
            <p className="text-xs sm:text-sm font-medium text-muted-foreground">{title}</p>
            <p className={`text-xl sm:text-2xl font-bold ${urgent ? 'text-orange-500' : ''}`}>
              {value.toLocaleString()}
            </p>
          </div>
          <div className={`flex h-8 w-8 sm:h-10 sm:w-10 items-center justify-center rounded-xl ${iconBg}`}>
...
function TenderListItem({ 
  ...
  return (
    <Link 
      ...
      className="flex items-start gap-3 p-3 -mx-3 rounded-xl hover:bg-muted/50 transition-colors group"
    ...
    >
      ...
      <div className="flex-1 min-w-0 space-y-1">
        ...
        <div className="flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
          ...
          <Badge className={`${getCategoryColor(tender.category)} text-[10px] px-1.5 py-0`}>
            {tender.category}
          </Badge>
        </div>
      </div>
...
function ClosingSoonCard({ tender, index }: { tender: Tender; index: number }) {
  ...
  return (
    <Link 
      href={`/tenders/${tender.id}`}
      className={`block p-4 rounded-xl border transition-all hover:shadow-md hover:-translate-y-0.5 group ${
        isUrgent ? 'border-red-500/30 bg-red-500/5' : 'hover:border-primary/30'
      }`}
      style={{ animationDelay: `${index * 50}ms` }}
    >
      <p className="font-medium text-sm line-clamp-2 group-hover:text-primary transition-colors mb-3">
        {tender.title}
      </p>
      <div className="flex items-center justify-between">
