...
exports.DigestExecutionAbortedError = exports.DigestExecutionCancelledError = void 0;
exports.executeDigestRun = executeDigestRun;
const supabase_js_1 = require("@supabase/supabase-js");
const email_1 = require("@/lib/email");
const app_url_1 = require("@/lib/app-url");
const config_1 = require("@/lib/supabase/config");
class DigestExecutionCancelledError extends Error {
    constructor(message = "Digest cancelled by admin.") {
        super(message);
        this.name = "DigestExecutionCancelledError";
    }
}
exports.DigestExecutionCancelledError = DigestExecutionCancelledError;
class DigestExecutionAbortedError extends Error {
    constructor(message = "Digest run is no longer active.") {
        super(message);
        this.name = "DigestExecutionAbortedError";
    }
}
exports.DigestExecutionAbortedError = DigestExecutionAbortedError;
...
async function getDigestRunControlState(digestId) {
...
function isCancellationRequested(metadata) {
...
function normalizeTender(row) {
    ...
    return {
        ...
        referenceNumber: typeof metadata.reference_number === "string" ? metadata.reference_number : null,
...
function parsePreferences(value) {
    ...
    const maxItems = typeof raw.maxItems === "number" && Number.isFinite(raw.maxItems)
...
function buildSearchText(tender) {
    return [
        tender.title,
        tender.summary ?? "",
        tender.issuer ?? "",
...
function matchesCategories(tender, categories) {
...
function filterTendersForSubscriber(tenders, preferences) {
    const filtered = tenders.filter((tender) => {
        ...
        if (preferences.keywordsInclude.length > 0 &&
...
function buildDigestSubject(tenderCount) {
...
async function updateDigestRun(options) {
    ...
    const failureSummary = options.failures.length > 0 ? options.failures.map((failure) => `${failure.email}: ${failure.message}`).join(" | ") : null;
    const { data, error } = await supabase
        .from("digest_runs")
        .update({
        ...
        logs,
        error_message: options.status === "fail"
...
async function executeDigestRun(options) {
    const supabase = getSupabaseAdmin();
    ...
    const tenders = (tendersResult.data ?? []).map(normalizeTender);
    ...
    const appBaseUrl = (0, app_url_1.getAppBaseUrl)();
    const dashboardUrl = `${appBaseUrl}/tenders`;
    const attempts = [];
    const finalizeCancellation = async (controlMetadata) => {
        const sentAttempts = attempts.filter((attempt) => attempt.status === "sent");
